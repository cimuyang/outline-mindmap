...
export interface StyleData {
   ...
   * 全局默认样式：没有单篇覆盖的笔记都用它。
   *
   * 【字段名不叫 global】——那是 Node 的全局对象名，官方审查的 avoid-global 规则会报，
   * 而且读代码时 `data.global` 一眼看不出是「全局样式」还是「全局对象」。
   */
  defaults: MindmapStyle
  /** key 是文件路径。改名要迁移、删除要清理（陷阱 7）。 */
  perFile: Record<string, MindmapStyle>
}

export function defaultStyleData(): StyleData {
  return { defaults: { ...DEFAULT_STYLE }, perFile: {} }
}
...
export function normalizeStyleData(raw: unknown): StyleData {
  const o = (raw ?? {}) as Record<string, unknown>
  const defaults = normalizeStyle(o['defaults'])
  ...
  if (src && typeof src === 'object') {
    for (const [path, value] of Object.entries(src as Record<string, unknown>)) {
      ...
      if (path) perFile[path] = normalizeStyle(value, defaults)
    }
  }
  return { defaults, perFile }
}
...
export class StyleStore {
  ...
  applyFile(path: string, style: MindmapStyle): void {
    if (sameStyle(style, this.data.defaults)) delete this.data.perFile[path]
