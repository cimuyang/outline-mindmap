...
import { fileToShow } from '../view/follow'
...
describe('fileToShow（拿到存档路径时显示哪一篇）', () => {
  const RESTORED = '存档的那篇'
  const ACTIVE = '活动笔记'

  it('没固定：活动笔记压过存档路径', () => {
    // 这就是「导图停在上一篇」的修复点：后台被卸载期间换过的笔记，
    // 存档路径是过时的，必须让活动笔记说了算。
    // 视图刚打开（onOpen）与活着时收到 setState 走的是同一条规则，见 follow.ts
    expect(fileToShow(RESTORED, ACTIVE, false)).toBe(ACTIVE)
  })

  it('没固定、也没有活动笔记：退回存档路径', () => {
    // 主区域里孤零零开着一个导图、一篇笔记都没开的情形，重启后还停在原来那篇
  ...
  it('固定、但还没有存档路径：跟活动笔记', () => {
    // 固定开着时新开一个导图，不能是空的（syncActiveFile 里 force 那一路的同一个道理）
    expect(fileToShow(null, ACTIVE, true)).toBe(ACTIVE)
  })

  it('两个候选都没有 → null，由调用方去画空状态', () => {
